...
% ==================== TITLE PAGE ====================
\title{\textbf{\Large From Threat Intelligence to Continuous Security Validation: A Unified Framework for Automated Threat Modeling and Risk-Oriented Penetration Testing}}
...
\author{
    ...
    AbdElRahman Hisham\textsuperscript{6},
    Dr.~Essam Shaaban\\[6pt]
    ...
    \textsuperscript{6}2205032@anu.edu.eg}}
}
...
The remainder of this paper is organized as follows. Section~II defines the research problem. Section~III reviews related work. Section~IV describes the proposed architecture. Section~V presents the validation and benchmark results. Section~VI presents the evaluation results. Section~VII concludes the paper.
...
This section presents the evaluation results of the TIBSA platform across its three primary modules: Threat Modeling, Penetration Testing, and URL and File Threat Detection.

\subsection{Threat Modeling Module Evaluation}

The threat modeling module was evaluated across four diverse system configurations to determine its accuracy and coverage against established industry benchmarks. The assessment included a review of the module's coverage of the OWASP Top 10 (2021) and the CWE Top 25.

\begin{table}[H]
\centering
\caption{Threat Modeling Test Cases Summary}
\label{tab:tm_test_cases}
\small
\renewcommand{\arraystretch}{1.3}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{0.3\textwidth}|c|p{0.35\textwidth}|p{0.2\textwidth}|}
\hline
\textbf{Test Case (Stack)} & \textbf{Threats Found} & \textbf{Key Findings} & \textbf{Estimated Accuracy} \\
\hline
1 --- Web/SaaS (React, FastAPI, PostgreSQL, Redis) & 18 $\rightarrow$ 17 & Initial: $\sim$70--75\% (duplicate CWEs, redundant threats) & $\sim$85--90\% (after fixes) \\
\hline
2 --- IoT/API (Express, NestJS, MQTT, C/C++, Edge) & 8 & Strong: correct CWE-306, CWE-798, CWE-265, CWE-319 & $\sim$85\% (Memory Safety \& Edge Cache Poisoning missing CWE) \\
\hline
3 --- Mobile B2C (Flask, Java, Rust, gRPC, WSS) & 9 & No false positives for DB threats (none selected). Rust correctly triggers no Memory Safety threat. & $\sim$88\% (no Mobile-specific threats e.g. insecure storage/reverse engineering) \\
\hline
4 --- Cloud Serverless (Django, Spring Boot, DynamoDB, Elasticsearch, AMQP) & 8 & Accurate Elasticsearch \& Serverless threats; DynamoDB \& AMQP-specific threats missing & $\sim$85\% (coverage gaps: DynamoDB, AMQP) \\
\hline
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{OWASP Top 10 (2021) Coverage Mapping}
\label{tab:tm_owasp}
\small
\renewcommand{\arraystretch}{1.3}
\resizebox{\textwidth}{!}{%
...
\hline
\textbf{OWASP Category} & \textbf{TIBSA Threats Mapped} & \textbf{Status} \\
\hline
A01: Broken Access Control & Broken Object-Level Authorization, Privilege Escalation, Tenant Data Isolation Failure & \cmark\ Covered \\
\hline
A02: Cryptographic Failures & Sensitive Data Exposure (encryption at rest/in transit) & \cmark\ Covered \\
\hline
A03: Injection & SQL/Query Injection, Database Poisoning via Third-Party Integration & \cmark\ Covered \\
\hline
A04: Insecure Design & Cloud Misconfiguration, Serverless Function Event Injection (partial) & $\sim$ Partial \\
\hline
A05: Security Misconfiguration & Cloud Misconfiguration, Redis/Elasticsearch Unauthenticated Access & \cmark\ Covered \\
\hline
A06: Vulnerable \& Outdated Components & Not generated in any test & \xmark\ Gap \\
\hline
A07: Identification \& Authentication Failures & Identity Spoofing, API Token Theft and Replay & \cmark\ Covered \\
\hline
A08: Software \& Data Integrity Failures & Database Poisoning via Third-Party Integration, Mass Assignment & \cmark\ Covered \\
\hline
A09: Security Logging \& Monitoring Failures & Insufficient Audit Logging for Administrative Actions & \cmark\ Covered (conditional) \\
\hline
A10: Server-Side Request Forgery (SSRF) & SSRF via REST APIs & \cmark\ Covered \\
\hline
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{CWE Top 25 Coverage Mapping}
\label{tab:tm_cwe}
\small
\renewcommand{\arraystretch}{1.3}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|l|l|}
\hline
\textbf{CWE ID} & \textbf{Weakness} & \textbf{Coverage Status} \\
\hline
CWE-79 & Cross-site Scripting (XSS) & \cmark\ Covered \\
\hline
CWE-787 & Out-of-bounds Write & \xmark\ Gap (Memory Safety threat lacks CWE) \\
\hline
CWE-89 & SQL Injection & \cmark\ Covered \\
\hline
CWE-352 & CSRF & \cmark\ Covered \\
\hline
CWE-22 & Path Traversal & \xmark\ Gap \\
\hline
CWE-125 & Out-of-bounds Read & \xmark\ Gap \\
\hline
CWE-78 & OS Command Injection & \xmark\ Gap \\
\hline
CWE-416 & Use After Free & \xmark\ Gap (mentioned in description only, no CWE tag) \\
\hline
CWE-862 & Missing Authorization & $\sim$ Partial (covered via BOLA/CWE-639) \\
\hline
CWE-434 & Unrestricted Upload of Dangerous File Type & \xmark\ Gap \\
\hline
CWE-918 & Server-Side Request Forgery (SSRF) & \cmark\ Covered \\
\hline
CWE-306 & Missing Authentication for Critical Function & \cmark\ Covered (Redis, MQTT, Elasticsearch) \\
\hline
CWE-862/863 & Incorrect/Missing Authorization & \cmark\ Covered (BOLA, Tenant Isolation) \\
\hline
CWE-798 & Use of Hard-coded Credentials & \cmark\ Covered (IoT Firmware test) \\
\hline
CWE-269 & Improper Privilege Management & \cmark\ Covered (Privilege Escalation) \\
\hline
CWE-200 & Exposure of Sensitive Information & \cmark\ Covered (gRPC Reflection, 3rd-party leakage) \\
\hline
CWE-732 & Incorrect Permission Assignment & \cmark\ Covered (Cloud Misconfiguration) \\
\hline
CWE-287 & Improper Authentication & \cmark\ Covered (Identity Spoofing) \\
\hline
CWE-94 & Code Injection & \xmark\ Gap \\
\hline
CWE-639 & Authorization Bypass Through User-Controlled Key (IDOR) & \cmark\ Covered (BOLA) \\
\hline
\end{tabular}%
}
\end{table}

The threat modeling module demonstrated an overall estimated accuracy of approximately 85--90\%. Key strengths included precision in CWE mapping, an absence of false positives for threats irrelevant to the selected technology stack, and correct implementation of conditional logic. However, notable coverage gaps were observed for DynamoDB, AMQP, and MongoDB. Furthermore, the application type did not adequately influence threat selection, and a notable deficiency was identified regarding vulnerable and outdated components (OWASP A06/SCA).

\subsection{Pentest Module Evaluation}

The penetration testing module was evaluated using aggressive-mode scans against two publicly vulnerable targets. The results were compared against an established benchmark scanner, ZeroThreat.

\begin{table}[H]
\centering
\caption{Pentest Test 1 --- Target: juiceshop.zerothreat.ai/juiceshop}
\label{tab:pt_test1}
\small
\renewcommand{\arraystretch}{1.3}
\resizebox{\textwidth}{!}{%
...
\hline
\textbf{Metric} & \textbf{TIBSA} & \textbf{ZeroThreat} & \textbf{Observation} \\
\hline
...
\begin{table}[H]
\centering
\caption{Pentest Test 2 --- Target: brokencrystals.zerothreat.ai (API scope)}
\label{tab:pt_test2}
\small
\renewcommand{\arraystretch}{1.3}
\resizebox{\textwidth}{!}{%
...
\textbf{Metric} & \textbf{TIBSA} & \textbf{ZeroThreat} & \textbf{Observation} \\
\hline
Total Findings & 23 & 107 (35+67+5) & ZeroThreat found $\sim$4.6x more issues \\
\hline
...
The penetration testing module achieved an overall estimated accuracy of 30--40\%. A critical structural issue was identified wherein the overall risk score formula appeared disconnected from the actual severity breakdown of findings, leading to misleadingly low scores on high-risk targets. Specifically, the module detected zero critical vulnerabilities on targets where the benchmark identified 35. Additionally, an attack-surface reporting bug resulted in contradictory metrics, such as reporting zero endpoints while simultaneously logging multiple findings.

\subsection{URL and File Threat Detection Module Evaluation}

The threat detection module was evaluated as a hybrid scanner combining VirusTotal intelligence across multiple engines with an AI-based threat model.

\begin{table}[H]
\centering
\caption{URL and File Threat Detection --- Comparative Scan Results}
\label{tab:vt_scans}
\small
\renewcommand{\arraystretch}{1.3}
\resizebox{\textwidth}{!}{%
...
\hline
\textbf{Target} & \textbf{Type} & \textbf{VT Detection} & \textbf{Threat Score} & \textbf{Verdict} \\
\hline
...
The scanner produced zero false positives on known-safe domains and correctly flagged malicious content in both test cases. However, evaluation revealed that the AI model contributed 0\% weight to the threat scores, meaning the final verdicts were driven entirely by VirusTotal data aggregation. This finding indicates that the AI component is not yet fully integrated into the file-type detection scoring pipeline.

\subsection{Evaluation Limitations}

\subsubsection{Threat Modeling Module}
A key limitation of the evaluation methodology was the absence of a directly comparable baseline tool. Existing automated threat modeling tools, such as OWASP Threat Dragon, do not accept a textual technology stack description and automatically generate threats in the same manner as the proposed platform. Instead, these tools require the user to manually construct a Data Flow Diagram (DFD) and subsequently select threats from a predefined library on a per-element basis, rather than generating stack-specific threats automatically. Consequently, a like-for-like, tool-to-tool comparison was not feasible. To address this limitation, the evaluation instead compared the generated threats against established, industry-recognized reference frameworks---namely the OWASP Top 10 (2021) and the CWE Top 25---which represent a widely accepted ground truth for assessing the relevance and completeness of identified threats.

\subsubsection{File Scanning Module}
A further limitation arose during the evaluation of the file-based threat detection module. Comprehensive testing would ideally involve a diverse set of malicious file samples representing different threat categories (e.g., scripts, executables, macros, and shell commands). However, sourcing and handling multiple malicious file samples poses a significant safety risk, as such files could inadvertently infect or compromise the testing device if not properly isolated. To mitigate this risk while still validating the module's detection capability, a single representative malicious test file was used, in addition to the standard EICAR test file---an industry-standard, intentionally harmless file specifically designed to trigger antivirus and malware detection engines without posing any actual risk. This approach allowed the detection accuracy of the module to be verified under controlled, safe conditions, although it limits the generalizability of the results across the full spectrum of possible malicious file types.
